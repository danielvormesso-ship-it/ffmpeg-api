...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    ...
    // ALWAYS pre-normalize input to safe H.264 baseline to prevent filter_complex failures
    // This eliminates auto_scale / Parsed_scale errors caused by exotic pixel formats, 
    // odd dimensions, or non-standard codec parameters
    if (!probeInfo.unsupportedCodec) {
      const normPath = path.join(UPLOAD_DIR, `${uuidv4()}_prenorm.mp4`);
      console.log(`Job ${jobId} pre-normalizing input to safe baseline (${probeInfo.rawWidth}x${probeInfo.rawHeight}, codec=${probeInfo.codecName})...`);
      try {
