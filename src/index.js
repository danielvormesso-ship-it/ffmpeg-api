...
const OUTPUT_WIDTH = Math.max(2, Number(process.env.OUTPUT_WIDTH || 1080));
const OUTPUT_HEIGHT = Math.max(2, Number(process.env.OUTPUT_HEIGHT || 1920));
const FINAL_CRF = String(process.env.FINAL_CRF || 22);
const FINAL_PRESET = String(process.env.FINAL_PRESET || 'veryfast');
const FINAL_AUDIO_BITRATE = String(process.env.FINAL_AUDIO_BITRATE || '160k');
const POPUP_CRF = String(process.env.POPUP_CRF || 24);
const POPUP_PRESET = String(process.env.POPUP_PRESET || 'veryfast');
...
async function transcodeUnsupportedInputToH264(inputPath, outputPath) {
  await runWithFfmpegQueue([
    '-hide_banner', '-loglevel', 'error', '-nostats', '-threads', '1',
    '-i', inputPath,
    '-map', '0:v:0', '-map', '0:a?',
    '-c:v', 'libx264', '-preset', FINAL_PRESET, '-crf', '24', '-pix_fmt', 'yuv420p',
    '-c:a', 'aac', '-b:a', FINAL_AUDIO_BITRATE,
    '-movflags', '+faststart', '-y', outputPath,
  ]);
}

async function ensureSupportedInputVideo(inputPath, probeInfo) {
  if (!probeInfo.unsupportedCodec) return probeInfo;
  const transcodePath = path.join(UPLOAD_DIR, `${uuidv4()}_transcoded.mp4`);
  try {
    await transcodeUnsupportedInputToH264(inputPath, transcodePath);
    ...
    return { ...newProbe, unsupportedCodec: false };
  } catch (err) {
    try { fs.unlinkSync(transcodePath); } catch {}
    const codecLabel = probeInfo.codecTag || probeInfo.codecName || 'desconhecido';
    throw new Error(`O vídeo usa codec ${codecLabel} que este servidor não consegue converter. Exporte em MP4 H.264 e tente novamente.`);
  }
}
...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    ...
    if (probeInfo.unsupportedCodec) {
      ...
      try {
        const newProbe = await ensureSupportedInputVideo(inputPath, probeInfo);
        ...
        updateJob(jobId, { status: 'failed', error: transcodeErr.message || detail });
        return;
      }
    }
...
app.post('/api/process', auth, upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    if (probeInfo.unsupportedCodec) {
      try {
        probeInfo = await ensureSupportedInputVideo(inputPath, probeInfo);
      } catch (err) {
        cleanup(inputPath, outputPath);
        return res.status(422).json({ error: err.message || 'Unsupported source codec' });
      }
    }
...
async function normalizePopupVideoAsset(filePath) {
  ...
  if (cached) {
    ...
    normalizedPopupCache.delete(filePath);
  }
  ...
  const highQualityCmd = [
    ...
    '-c:v', 'libx264', '-preset', POPUP_PRESET, '-crf', POPUP_CRF,
    '-pix_fmt', 'yuv420p', '-movflags', '+faststart',
    '-y', normalizedPath,
  ];

  try {
    console.log(`[Normalize] Attempt 2: high-quality re-encode at ${OUTPUT_WIDTH}x${OUTPUT_HEIGHT}`);
    await runWithFfmpegQueue(highQualityCmd);
    if (fs.existsSync(normalizedPath) && fs.statSync(normalizedPath).size > 1024) {
      normalizedPopupCache.set(filePath, normalizedPath);
      console.log('[Normalize] High-quality re-encode succeeded');
      return normalizedPath;
    }
    throw new Error('Output too small after high-quality re-encode');
  } catch (errB) {
    attemptErrors.push(`hq_reencode: ${getExecErrorDetails(errB)}`);
    try { fs.unlinkSync(normalizedPath); } catch {}
  }
  ...
  try {
    console.log('[Normalize] Attempt 3: fallback re-encode at 720x1280');
    await runWithFfmpegQueue(lightCmd);
    if (fs.existsSync(normalizedPath) && fs.statSync(normalizedPath).size > 1024) {
      normalizedPopupCache.set(filePath, normalizedPath);
      console.log('[Normalize] Fallback re-encode succeeded');
      return normalizedPath;
    }
    throw new Error('Output too small after fallback re-encode');
  } catch (errC) {
    attemptErrors.push(`fallback_reencode: ${getExecErrorDetails(errC)}`);
    try { fs.unlinkSync(normalizedPath); } catch {}
    if (errC.isOOM) {
      throw new Error(`Popup normalization killed by OS (OOM). Attempts: ${attemptErrors.join(' | ').slice(0, 2500)}`);
    }
    throw new Error(`Popup normalization failed. Attempts: ${attemptErrors.join(' | ').slice(0, 2500)}`);
  }
}
...
async function probeVideo(filePath) {
  try {
    ...
    return { unsupportedCodec: false, hasAudio: true, codecName: '', codecTag: '', width: OUTPUT_WIDTH, height: OUTPUT_HEIGHT, probeFailed: true };
  }
}
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  const baseW = OUTPUT_WIDTH;
  const baseH = OUTPUT_HEIGHT;
  ...
  const cmd = ['-hide_banner', '-loglevel', 'error', '-nostats', '-threads', '1', ...inputs];
  ...
  const shouldCopyVideo = !needsVideoEncode && totalDuration <= 0;
  ...
  if (audioOut) {
    cmd.push('-c:a', 'aac', '-b:a', FINAL_AUDIO_BITRATE);
  }
