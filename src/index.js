...
const MAX_CONCURRENT_FFMPEG = Math.max(1, Number(process.env.MAX_CONCURRENT_FFMPEG || 3));
...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    // Download video with retry for transient failures (0 bytes, network errors)
    ...
    while (true) {
      ...
      try {
        ...
        if (downloadAttempts >= MAX_DOWNLOAD_ATTEMPTS) {
          throw new Error(`Download falhou após ${MAX_DOWNLOAD_ATTEMPTS} tentativas: ${dlErr.message}`);
        }
        console.warn(`Job ${jobId} download attempt ${downloadAttempts} failed: ${dlErr.message}. Retrying in 2s...`);
    ...
    // ALWAYS pre-normalize input to stable 720x1280 h264/yuv420p baseline
    // This prevents filter_complex failures caused by unusual pixel formats,
    // variable frame rates, odd dimensions, or exotic color spaces in TikTok videos
    ...
    try {
      ...
      console.warn(`Job ${jobId} pre-normalize failed:`, normErr.message);
      // If pre-normalize fails, the video is likely too broken to process
      throw new Error(`Falha na normalização do vídeo: ${getExecErrorDetails(normErr).slice(0, 500)}`);
    }
    ...
    try {
      ...
      // NO FALLBACKS: if primary processing fails, the video is rejected.
      // Delivering a video without popup or without audio is worse than skipping it.
      throw new Error(
