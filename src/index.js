...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    ...
    // Pre-normalize: if video has odd dimensions or unusual codec params, re-encode to safe baseline
    const needsPreNormalize = !probeInfo.unsupportedCodec && (
      (probeInfo.rawWidth && probeInfo.rawWidth % 2 !== 0) ||
      (probeInfo.rawHeight && probeInfo.rawHeight % 2 !== 0) ||
      probeInfo.codecName === 'vp9' ||
      probeInfo.codecName === 'av1' ||
      probeInfo.codecName === 'theora'
    );

    if (needsPreNormalize) {
      const normPath = path.join(UPLOAD_DIR, `${uuidv4()}_prenorm.mp4`);
      console.log(`Job ${jobId} pre-normalizing input (${probeInfo.rawWidth}x${probeInfo.rawHeight}, codec=${probeInfo.codecName})...`);
      try {
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  if (usePopupMedia) {
    ...
    // Use width% to define the popup box size, keeping aspect ratio via force_original_aspect_ratio
    const scaleW = Math.max(2, Math.round((baseW * transformW / 100) / 2) * 2);
    // Scale height = -1 lets FFmpeg auto-calculate to preserve aspect ratio
    ...
    if (isPopupVideo) {
      if (hasCustomTransform) {
        filterParts.push(
          `${overlayInputLabel}scale=${scaleW}:-2,fps=30,setsar=1,format=yuva420p${rotateFilter}[ovr]`,
      ...
      if (hasCustomTransform) {
        const overlayBaseFilter = simplifiedOverlay
          ? `${overlayInputLabel}scale=${scaleW}:-2,setsar=1,format=yuva420p${rotateFilter}[ovr]`
          : `${overlayInputLabel}scale=${scaleW}:-2,format=rgba,colorchannelmixer=aa=${opacityVal}${rotateFilter}[ovr]`;
        filterParts.push(
          overlayBaseFilter,
