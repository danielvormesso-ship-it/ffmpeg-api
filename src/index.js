...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  if (usePopupMedia) {
    ...
    // Use explicit width AND height to match the preview editor exactly
    ...
    if (isPopupVideo) {
      if (hasCustomTransform) {
        filterParts.push(
          `${overlayInputLabel}scale=${scaleW}:${scaleH},fps=30,setsar=1,format=yuva420p${rotateFilter}[ovr]`,
      ...
      if (hasCustomTransform) {
        const overlayBaseFilter = simplifiedOverlay
          ? `${overlayInputLabel}scale=${scaleW}:${scaleH},setsar=1,format=yuva420p${rotateFilter}[ovr]`
