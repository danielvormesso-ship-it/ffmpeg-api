  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn(`Job ${jobId} resilient fallback failed:`, getExecErrorDetails(resilientErr));

        // Last resort: complete the video without popup media (faster + stable)
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  if (usePopupMedia) {
    ...
    const overlayBaseFilter = isPopupVideo
      ? `${overlayInputLabel}setpts=PTS[ovr]`
      : (simplifiedOverlay
  ...
  if (!skipAllAudio) {
    ...
    const needsAudioMix = hasPopupAudio || hasBgMusic || (useSourceAudio && videoVolumeAfterPopup < 100 && usePopupMedia);
