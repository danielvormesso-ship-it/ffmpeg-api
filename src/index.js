...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    ...
    if (requiresPopup) {
      ...
      if (!config?.endVideoWithPopup && Number.isFinite(probeInfo?.duration) && probeInfo.duration <= popupAppearAt + 0.05) {
      ...
      if (!config?.popupFullscreen && config?.popupTransform) {
        const t = config.popupTransform;
        const tx = Number(t.x ?? 0);
        const ty = Number(t.y ?? 0);
        const tw = Number(t.width ?? 100);
        const th = Number(t.height ?? 100);

        if (!Number.isFinite(tx) || !Number.isFinite(ty) || !Number.isFinite(tw) || !Number.isFinite(th)) {
          throw new Error('Transform do popup inválido (NaN/Infinity).');
        }

        if (tw <= 0 || th <= 0 || tx < 0 || ty < 0 || tx + tw > 100 || ty + th > 100) {
          throw new Error('Transform do popup inválido (fora da área visível).');
        }
      }
    }
