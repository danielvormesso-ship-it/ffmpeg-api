  ...
  try {
    ...
    try {
      ...
      if (sanitizedAssets.popupMedia) {
        ...
        usedSafeAudioFallback = true;
        const safeCmd = buildFFmpegCommand(inputPath, outputPath, config, sanitizedAssets, probeInfo, { skipAllAudio: true });
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  if (assets.popupMedia) {
    ...
    if (isPopupVideo) {
      // Delay popup video to appearAt and limit to popupDuration
      inputs.push('-itsoffset', String(appearAt), '-t', String(popupDuration), '-i', assets.popupMedia);
    } else {
      inputs.push('-i', assets.popupMedia);
    }

    const overlayInputLabel = `[${inputIdx}:v]`;
    const overlayBaseFilter = isPopupVideo
      ? `${overlayInputLabel}fps=30,scale=1080:1920:force_original_aspect_ratio=decrease,setsar=1,format=yuv420p[ovr]`
      : (simplifiedOverlay
        ? `${overlayInputLabel}scale=1080:1920:force_original_aspect_ratio=decrease[ovr]`
        : `${overlayInputLabel}scale=1080:1920:force_original_aspect_ratio=decrease,format=rgba,colorchannelmixer=aa=${opacityVal}[ovr]`);

    if (isPopupVideo) {
      filterParts.push(
        overlayBaseFilter,
        `[0:v][ovr]overlay=(W-w)/2:(H-h)/2[vout]`
      );
    } else {
