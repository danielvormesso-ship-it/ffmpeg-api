...
app.post('/api/process', auth, upload.single('video'), async (req, res) => {
  ...
  try {
    ...
    const shouldRetrySafeAudio = !!assets?.popupMedia;
    let usedSafeAudioFallback = false;

    try {
      await runWithFfmpegQueue(cmd);
    } catch (primaryErr) {
      console.warn('Primary FFmpeg failed:', getExecErrorDetails(primaryErr));
      // Retry without any audio (video overlay only)
      ...
      console.warn('Retrying with skipAllAudio mode.');
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  // Audio handling - KEY FIX: when volume is 0, skip source audio entirely
  if (!skipAllAudio) {
    ...
    if (needsAudioMix) {
      ...
      if (useSourceAudio) {
        if (videoVolumeAfterPopup < 100 && assets.popupMedia) {
          ...
          filterParts.push(`[0:a]volume=${volAfter}:enable='gte(t,${appearAt})'[a_orig]`);
        } else {
      ...
      if (audioLabels.length === 0) {
        // No audio sources available, no audio output
        audioOut = null;
  ...
  // skipAllAudio: audioOut stays null, no audio in output
