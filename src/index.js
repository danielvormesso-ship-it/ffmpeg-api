...
const FFMPEG_QUEUE_WAIT_TIMEOUT_MS = Math.max(15000, Number(process.env.FFMPEG_QUEUE_WAIT_TIMEOUT_MS || 2 * 60 * 1000));
...
async function runWithJobHeartbeat(jobId, minimumProgress, task) {
  const heartbeat = setInterval(() => {
    ...
    updateJob(jobId, {
      ...
      progress: Number.isFinite(currentProgress)
        ? Math.max(minimumProgress, currentProgress)
        : minimumProgress,
    });
...
async function processJobAsync(jobId, videoUrl, config, assets) {
  ...
  try {
    ...
    try {
      ...
      await runWithJobHeartbeat(jobId, 60, () => runWithFfmpegQueue(cmd));
    } catch (primaryErr) {
      ...
      if (shouldTryRecovery) {
        try {
          ...
          await runWithJobHeartbeat(jobId, 70, () => runWithFfmpegQueue(singleThreadRecoveryCmd));
...
function buildFFmpegCommand(inputPath, outputPath, config, assets, probeInfo = {}, options = {}) {
  ...
  if (usePopupMedia) {
    ...
    if (isPopupVideo) {
      ...
      inputs.push('-loop', '1', '-framerate', '30', '-i', assets.popupMedia);
    }
