...
async function runWithFfmpegQueue(cmd) {
  ...
  try {
    ...
    if (canFallbackEncoder) {
      ...
      console.warn('[FFmpeg] Fallback command:', fallbackCmd.join(' '));
      const fallbackResult = await execFileAsync('ffmpeg', fallbackCmd, {
        timeout: 900000,
        maxBuffer: 20 * 1024 * 1024,
      });
...
async function normalizePopupVideoAsset(filePath) {
  ...
  try {
    ...
    attemptErrors.push(`normalize_vf: ${getExecErrorDetails(errA)}`);

    const normalizeSimpleCmd = [
      '-hide_banner', '-loglevel', 'error', '-nostats',
      '-i', filePath,
      '-an',
      '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '30',
    ...
    await runWithFfmpegQueue(normalizeSimpleCmd).catch((errB) => {
      attemptErrors.push(`normalize_simple: ${getExecErrorDetails(errB)}`);
      throw new Error(attemptErrors.join(' | ').slice(0, 3000));
    });
  }
  ...
  if (usePopupMedia) {
    ...
    if (isPopupVideo) {
      if (simpleVideoOverlay) {
        // Simplest possible path to avoid filter_complex crashes on problematic popup videos
        filterParts.push(
          `${overlayInputLabel}fps=30,format=yuv420p[ovr]`,
          `[0:v][ovr]overlay=(W-w)/2:(H-h)/2:eof_action=pass[vout]`
        );
      } else {
        // Scale+pad popup to exactly match base video resolution, force compatible format
        filterParts.push(
          `${overlayInputLabel}scale=${baseW}:${baseH}:force_original_aspect_ratio=increase,crop=${baseW}:${baseH}:(iw-${baseW})/2:(ih-${baseH})/2,setsar=1,format=yuv420p[ovr]`,
          `[0:v][ovr]overlay=0:0:eof_action=pass[vout]`
        );
      }
    } else {
      const overlayBaseFilter = simplifiedOverlay
        ? `${overlayInputLabel}scale=${baseW}:${baseH}:force_original_aspect_ratio=increase,crop=${baseW}:${baseH}:(iw-${baseW})/2:(ih-${baseH})/2[ovr]`
        : `${overlayInputLabel}scale=${baseW}:${baseH}:force_original_aspect_ratio=increase,crop=${baseW}:${baseH}:(iw-${baseW})/2:(ih-${baseH})/2,format=rgba,colorchannelmixer=aa=${opacityVal}[ovr]`;
