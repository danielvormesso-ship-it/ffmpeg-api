  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Extra resilient: popup only, no audio mix, no effects overlay
        try {
          const extraCmd = buildFFmpegCommand(inputPath, outputPath, config, {
            ...sanitizedAssets,
            popupAudio: null,
            bgMusic: null,
            effectsOverlay: null,
          ...
          console.log(`Job ${jobId} FFmpeg extra-resilient (popup only):`, extraCmd.join(' '));
          await runWithFfmpegQueue(extraCmd);
        } catch (extraErr) {
          const extraDetails = getExecErrorDetails(extraErr);
          attemptErrors.push(`extra_resilient: ${extraDetails}`);
          console.warn(`Job ${jobId} extra-resilient failed, emergency passthrough...`);
          try { fs.unlinkSync(outputPath); } catch {}

          // Final fallback: no popup at all
